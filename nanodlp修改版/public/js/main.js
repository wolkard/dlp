...
function update_timeline(){
	...
	if (update_status.running){
		...
		last_value('last_location', (last_value('layers_count')-current_layer_id)+" 层");
		...
		last_value('last_remaining',remaining_time+" 分");
...
function current_status_display(){
	...
	if(last_value('path').length>10){
		$(".last_path").html(last_value('path').substr(0,8)+'......');		
	}
	else{
